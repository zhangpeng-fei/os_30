...
void init_screen8(short *vram, int x, int y)
{
	...
	boxfill8(vram, x, COL8_848484,  0,     y - 28, x -  1, y-1);
	boxfill8(vram, x, COL8_C6C6C6,  1,     y - 27, x -  2, y-2);
	
	boxfill8(vram, x, COL8_FFFFFF,  2, y - 24, 59, y - 24);
	boxfill8(vram, x, COL8_FFFFFF,  2, y - 23,  2, y -  4);
	boxfill8(vram, x, COL8_848484,  3, y -  4, 59, y -  4);
	boxfill8(vram, x, COL8_848484, 59, y - 22, 59, y -  5);
	...
	for (j = 0; j < 13; j++) {
		for (i = 0; i < 44; i++) {
			...
			if (c != 0xff) {
				vram[(y - 20 + j) * x + (10 + i)] = table_8_565[c];
			}
		}
	}
	return;
