...
void fdc_task(void)
{
	...
	io_out8(0x03f2, 0x0c);	/* OS起動後に1度FDDのモーター停止 */
	...
	for (;;) {
		...
		if (fifo32_status(&task->fifo) == 0) {
			...
			io_sti();
			if (i == 2880) {
				io_out8(0x03f2, 0x0c);	/* FDDのモーター停止 */
				mot = 0;
			} else {
				timer_cancel(timer);
				rw    = i >> 12;
...
int fdc_readwrite(int rw, int secno, struct FDCSV *sv, int *mot)
{
	...
	if (sv->rw == 1) {	/* 読み込みモード */
		sprintf(msg, "fdc read  -- C:%02d H:%02d S:%02d N:%02d",
				sv->cyl, sv->hed, sv->sec, sv->num);
		fdc_setread (sv->num);
	} else {			/* 書き込みモード */
		sprintf(msg, "fdc write -- C:%02d H:%02d S:%02d N:%02d",
				sv->cyl, sv->hed, sv->sec, sv->num);
		fdc_setwrite(sv->num);
	}
