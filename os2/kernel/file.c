...
{
	int i, secno, cyl, hed;
	int svcn, svsz, ret_flg = 0;

	for (;;) {
		secno = 0x3e00 / 512 + clustno;
		cyl = (int) secno / 36; secno %= 36;
		hed = (int) secno / 18; secno %= 18;
		if ((i_sta[cyl][hed] & (1 << secno)) != 0) {
			if (size <= 512) {
				if (ret_flg == 0) {
					for (i = 0; i < size; i++) {
...
void msec_sleep(int msec)
{
	...
	struct FIFO32 fifo;
	int i, fbuf[8];
	int stime = (int) ((msec + 9) / 10);
	fifo32_init(&fifo, 8, fbuf, task);
	timer_init(timer, &fifo, 1);
	timer_settime(timer, stime);
