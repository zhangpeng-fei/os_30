	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (256 <= i && i <= 511 || 4352 <= i && i <= 4607) { /* キーボードデータ */
				...
				if ('A' <= s[0] && s[0] <= 'Z') {	/* 大写和小写的?? */
					if (((key_leds & 4) == 0 && key_shift == 0) ||
							((key_leds & 4) != 0 && key_shift != 0)) {
						s[0] += 0x20;	/* 大文字を小文字に変換 */
					}
				}
				if (s[0] != 0 && shtctl->key_win != 0) { /* 一般字符、退格?、回??r */
				...
				if (i == 256 + 0x3b && key_shift != 0 && shtctl->key_win != 0) {	/* Shift+F1 */
					...
					if (task != 0 && task->tss.ss0 != 0) {
						...
						io_sti();
						task_run(task, -1, 0);	/* ?了???行?束?理，如果?于休眠状???醒す */
					}
				}
			...
			} else if (768 <= i && i <= 1023) {	/* 命令行窗口???理 */
				close_console(shtctl->sheets0 + (i - 768));
			} else if (1024 <= i && i <= 2023) {
				close_constask(taskctl->tasks0 + (i - 1024));
			} else if (2024 <= i && i <= 2279) {	/* 只??命令行窗口 */
				sht2 = shtctl->sheets0 + (i - 2024);
				memman_free_4k(memman, (int) sht2->buf, xxx * yyy);
				sheet_free(sht2);
...
void keywin_off(struct SHEET *key_win)
{
	...
	if ((key_win->flags & 0x20) != 0) {
		fifo32_put_io(&key_win->task->fifo, 3); /* 命令行窗口光?OFF */
	}
	return;
}
...
void keywin_on(struct SHEET *key_win)
{
	...
	if ((key_win->flags & 0x20) != 0) {
		fifo32_put_io(&key_win->task->fifo, 2); /* 命令行窗口光?ON */
	}
	ctl->key_win = key_win;
...
struct SHEET *open_console(struct SHTCTL *shtctl, unsigned int memtotal)
{
	...
	unsigned short *buf = (unsigned short *) memman_alloc_4k(memman, xxx * yyy * 2);
	sheet_setbuf(sht, buf, xxx, yyy, -1); /* 无透明色 */
	make_window8(buf, xxx, yyy, "console", 0);
	make_textbox8(sht, 5, 24, xxx-6, yyy-6, COL8_000000);
...
void close_console(struct SHEET *sht)
{
	...
	memman_free_4k(memman, (int) sht->buf, xxx * yyy * 2);
	sheet_free(sht);
	close_constask(task);
	return;
}
