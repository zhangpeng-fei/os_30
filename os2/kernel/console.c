...
struct FILEINFO *app_search(char *name, struct FILEINFO *finfo, int *fat);
...
void cmd_del(struct CONSOLE *cons, char *cmdline, int* fat)
{
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
	...
	/* ファイルを探す */
	finfo = app_search(name, (struct FILEINFO *) (ADR_DISKIMG + 0x002600), fat);
	if (finfo != 0 && (finfo->type & 0x18) == 0) {
		...
		if (appsiz >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00) {
			...
			for (i = 0; i < sizeof(task->name); i++) {
				if ('A' <= task->name[i] && task->name[i] <= 'Z') {
					task->name[i] += 0x20;
				} 
			}
...
struct FILEINFO *app_search(char *name, struct FILEINFO *finfo, int *fat)
{
	char *offset, fname[256];
	struct FILEINFO *ffinfo;

	memset(fname, 0, sizeof(fname));
	strncpy(fname, name, 255);
	ffinfo = file_search2(fname, finfo, 224, fat);
	if (ffinfo == 0 && fname[strlen(fname)] != '.' && strlen(fname) <= 251) {
	...
	if (ffinfo == 0) {
		...
		for (offset = epath; *offset != 0;) {
			memset(fname, 0, sizeof(fname));
			strncpy(fname, offset, 255);
			strncat(fname, name, 255 - strlen(fname));
			ffinfo = file_search2(fname, finfo, 224, fat);
			if (ffinfo == 0 && fname[strlen(fname)] != '.' && strlen(fname) <= 251) {
				/* 見つからなかったので後ろに".HRB"をつけてもう一度探してみる */
				strcat(fname, ".HRB");
	...
	for (;;) {
		...
		if (fifo32_status(&task->fifo) == 0) {
			...
			if (i == 1) {
				...
				putfouts8_asc_sht(sht, sht->bxsize - 135, sht->bysize - 21,COL8_000000, COL8_C6C6C6, s, 10);	
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	if (edx == 1) {
		...
		if (i < 8) {
			reg[7] = hrb_api_fopen((char *) ebx + ds_base, eax, &task->fhandle[i], task->fat);
		} else {
			reg[7] = 0;
		}
	} else if (edx == 34) {
